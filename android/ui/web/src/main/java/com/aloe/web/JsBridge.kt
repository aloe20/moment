...
class JsBridge {
    ...
    private var messageHandlers: MutableMap<String?, (String?, (String) -> Unit) -> Unit> =
        mutableMapOf()
    ...
    fun shouldOverrideUrlLoading(view: WebView?, url: String?): Boolean {
        ...
        return if (url2 == null) {
            false
        } else if (url2.startsWith(YY_RETURN_DATA)) {
            handlerReturnData(url2)
            true
        } else if (url2.startsWith(YY_OVERRIDE_SCHEMA)) {
            view?.also { flushMessageQueue(it) }
            true
        } else {
            false
        }
    ...
    private fun getDataFromReturnUrl(url: String): String? {
        return if (url.startsWith(YY_FETCH_QUEUE)) {
            url.replace(YY_FETCH_QUEUE, EMPTY_STR)
        } else {
                ...
                if (!list.isNullOrEmpty()) {
                    for (i in list.indices) {
                        ...
                        if (responseId.isNullOrEmpty()) {
                            ...
                            (if (m.handlerName.isNullOrEmpty()) defaultHandler else messageHandlers[m.handlerName])?.takeUnless { callbackId.isNullOrEmpty() }
                                ?.invoke(m.data) { data ->
